...
@Component({
  selector: 'export-feedback-files',
  templateUrl: './export-feedback-file.html',
  providers: [{ provide: 'Window', useValue: window }],
})
export class ExportFeedbackFileComponent
  ...
  implements OnInit
{
  ...
  constructor(
    ...
    public feedbackTransferService: FeedbackTransferService,
  ...
  ngOnInit(): void {
    ...
    this.files_row = this.currentItem['list']
    this.files_column = []
    this.files_column = [
      {
        title: this.translate.instant('transfer.accountFrom'),
        dataKey: 'accountFrom',
      },
      {
        title: this.translate.instant('transfer.accountTo'),
        dataKey: 'accountTo',
      },
      {
        title: this.translate.instant('transfer.feedback.beneficiaryName'),
        dataKey: 'beneficiaryName',
      },
      { title: this.translate.instant('transfer.amount'), dataKey: 'amount' },
      { title: this.translate.instant('transfer.remarks'), dataKey: 'remarks' },
      {
        title: this.translate.instant('transfer.feedback.result'),
        dataKey: 'returnCode',
      },
    ]
  }
  ...
  getXlsx() {
    ...
    const files_row = this.prepareRows(this.files_column, this.files_row)
    ...
    XLSX.utils.sheet_add_json(ws, files_row, {
      skipHeader: false,
      origin: { r: 4, c: 0 },
    })

    // this.changeHeaderTitle(ws, this.level_column,11);
    this.changeHeaderTitle(ws, this.files_column, 5)
        ...
        { text: '\n\n' },
        {
          columns: [
            { width, text: '' },
            {
              ...
              table: {
                headerRows: 1,
                body: this.getTable(tAlignment, this.files_column, files_row),
              },
              layout: {
                defaultBorder: false,
                fillColor(i, node) {
  ...
  private getTableDetail(talign) {
    ...
    result.push(
      this.getLine(
        ...
        this.getDetailText(this.currentItem['fileReference'], talign),
        talign,
      ),
    )
    const feedbackDate = new DateFormatPipe(
      this.injector,
      this._locale,
    ).transform(this.currentItem['date'], 'dd/MM/yyyy')
    result.push(
      this.getLine(
        this.getDetailTitle(this.translate.instant('public.date'), talign),
        this.getDetailText(feedbackDate, talign),
        talign,
      ),
    )
    return result
