...
@Injectable()
export class RequestStatusPayments {
  token: string
  servicesUrl: string
  currentUser
  ...
  public getResults(page: number, rows: number): Observable<any> {
    ...
    return this.http
      .post(this.servicesUrl + '/moiPayment/requestStatus/payment/list', req)
      .pipe(
        map((response: any) => {
          const body = response
          ...
          if (response.errorCode !== '0') {
            ...
            const output = body.requestStatusEgovSPList.items
            /*
                    //console.log("--------------------------------");
                    //console.log(body.requestStatusEgovSPList.items);
                    //console.log("--------------------------------");
                    */
            ...
            pageObject.size = body.requestStatusEgovSPList.size
            pageObject.totalElements = body.requestStatusEgovSPList.total
            pageObject.totalPages =
              pageObject.totalElements / pageObject.pageSize

            pagedData.data = body.requestStatusEgovSPList.items
