...
export class RequestStatusService {
  ...
  getManagementData(searchData, page, rows): Observable<any> {
    const data: any = {
  ...
  getRequestData(searchData, page, rows): Observable<any> {
    const data: any = {
      dateFrom: this.datePipe.transform(searchData.dateFrom, 'yyyy-MM-dd'), //"string",
      dateTo: this.datePipe.transform(searchData.dateTo, 'yyyy-MM-dd'), //"string",
      status: searchData.status,
    ...
    return this.http
      .post(
        this.servicesUrl + '/posManagement/requestStatus/request/list',
        body,
      )
      .pipe(
        map((response: any, index: number) => {
          ...
          if (output.errorCode !== '0') {
            ...
            result = output
            result.posRequestBatchList.items.forEach((a) => {
  ...
  getMaintenanceData(searchData, page, rows): Observable<any> {
    const data: any = {
      dateFrom: this.datePipe.transform(searchData.dateFrom, 'yyyy-MM-dd'), //"string",
      dateTo: this.datePipe.transform(searchData.dateTo, 'yyyy-MM-dd'), //"string",
      status: searchData.status,
    ...
    return this.http
      .post(
        this.servicesUrl + '/posManagement/requestStatus/maintenance/list',
        body,
      )
      .pipe(
        map((response: any, index: number) => {
          ...
          if (output.errorCode !== '0') {
            ...
            result = output
            result.posMaintenanceBatchList.items.forEach((a) => {
              a['posMaintenanceRequestType'] = a['typeRequest']
            })
            result.error = false
          }
          return result
  ...
  getClaimData(searchData, page, rows): Observable<any> {
    const data: any = {
