...
export class ManageRequestService {
  ...
  public validate(datos, branch): Observable<any> {
    const data = {
      branch,
      city: datos.city,
    ...
    if (datos.terminalNumber) {
      datos.terminalNumber.forEach((a) => {
        data['terminals'].push(a['terminalNumber'])
      })
    }

    return this.http
      .post(this.servicesUrl + '/posManagement/maintenance/validate', data)
      .pipe(
        map((response: any) => {
          const body = response
          if (response.errorCode != 0) {
  ...
  public confirm(datos, requestValidate): Observable<any> {
    ...
    return this.http
      .post(this.servicesUrl + '/posManagement/maintenance/confirm', data)
      .pipe(
        map((response: any) => {
          const body = response
          if (response.errorCode != 0) {
    ...
    return this.http.get(url).pipe(
      map((response: any) => {
        ...
        if (response.errorCode != 0) {
          ...
          const result = output
          result['accountListDTO'] = response.listAccount
          return result
        }
      }),
      catchError(this.handleError),
