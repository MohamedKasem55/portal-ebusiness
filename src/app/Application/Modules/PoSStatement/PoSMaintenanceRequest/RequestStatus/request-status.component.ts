  ...
  implements OnInit
{
  ...
  urlActivate = [
    '/posstatement/pos-maintenance-request/request-status/activate',
  ]
  ...
  setPage(pageInfo) {
    ...
    this.requestStatusSubscription = this.requestStatusService
      ...
      .subscribe((result) => {
        if (!result.error) {
          this.requestStatus.elements = result.posMaintenanceBatchList.items
          this.requestStatus.size = result.posMaintenanceBatchList.size
          this.requestStatus.total = result.posMaintenanceBatchList.total
