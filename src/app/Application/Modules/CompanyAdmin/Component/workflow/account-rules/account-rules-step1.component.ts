...
export class AccountRulesStep1Component implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.companyLimit = this.storage.retrieve('company')['companyLimits']
    if (!storageVal) {
      return
    }
  ...
  accountChange(event) {
    ...
    if (event) {
      ...
      this.services.getAccountRules(event).subscribe((result: any) => {
        if (
  ...
  createAccountTableRules(form: FormGroup, accountRules) {
    ...
    accountRules.workflowTypePaymentList.forEach((account: any, index) => {
      accountControl.push(this.initAccountRules(account))
  ...
  initRules(ruleControl, rules, index) {
    const formGroup = this.fb.group(
      {
        amountMax: [
          rules.amountMax <= this.companyLimit ? rules.amountMax : null,
          {
            validators: Validators.compose([Validators.min(rules.amountMin), Validators.max(this.companyLimit)]),
            updateOn: 'blur',
          },
        ],
        amountOldMax: [rules.amountMax <= this.companyLimit ? rules.amountMax : null],
        amountMin: [
  ...
  addRule(rowIndex) {
    const ruleControl =
      this.form.controls.accountRules['controls'][rowIndex].controls.rules
  ...
  isAmountSet(privilegeIndex) {
    const ruleControl =
      this.form.controls.accountRules['controls'][privilegeIndex].controls.rules
    const index = ruleControl.length - 1
    return !(ruleControl.controls[index].controls['amountMax'].value != null && ruleControl.controls[index].valid)
  }
  ...
  adjustMinMaxControls(formGroupJ: number, formGroupI: number) {
    const controls =
      this.form.controls.accountRules['controls'][formGroupJ].controls.rules
        .controls
    ...
    for (let i = 0; i < controls.length; i++) {
      ...
      controls[i].controls.valid.setValue(
        controls[i].controls.amountMin.value <
          ...
          controls[i].controls.amountMax.value <=
            this._parseFloat(this.companyLimit) &&
