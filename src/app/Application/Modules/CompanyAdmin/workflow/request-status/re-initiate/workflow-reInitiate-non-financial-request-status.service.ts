...
@Injectable()
export class WorkflowReInitiateNonFinancialRequestStatusService extends AbstractActionModifyService {
    ...
    configureReinitiateFormModel(detailsData, futureLevels = false) {
        ...
        _fieldsConfigForForm.push({
            ...
            validators: [],
            widget: 'text',
            widget_container_class: 'col-xs-12 col-sm-1',
            widget_container_init_row: false,
            widget_container_end_row: false,
    ...
    protected createValidateRequest(values: any): Observable<any> {
        ...
        return this.http.post(
            ...
            '/workflow/requestStatus/nonFinancial/validate',
            params,
        ).pipe(
            map((result: any) => {
                ...
                // return result['workflowNonFinancialBatch'];
                return result;
            })
        )
    }
    ...
    protected createConfirmRequest(values: any): Observable<any> {
        const params = {
            workflowNonFinancialBatch: this.validateResponse.workflowNonFinancialBatch,
            requestValidate: this.validateResponse.requestValidate ? this.validateResponse.requestValidate : {}
        }
        return this.http.post(
            this.config.getServicesUrl() +
            '/workflow/requestStatus/nonFinancial/confirm',
            params,
        ).pipe(
            map((result: any) => {
                if (result && result.errorCode == "0") {
    ...
    public deleteConfirmRequest(values: any): Observable<any> {
        const params = { workflowNonFinancialBatch: values }
