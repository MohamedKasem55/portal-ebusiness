...
@Component({
  selector: 'export-pa-direct-debit',
  templateUrl: './export-pa-direct-debit-payroll.html',
  providers: [{ provide: 'Window', useValue: window }],
})
export class ExportPADirectDebitComponent
  ...
  constructor(
    ...
    public directDebitService: DirectDebitsService,
  ...
  ngOnInit(): void {
    ...
    this.records_row = this.currentItem['recordsData'].data

    if (this.currentItem['directDebit'] == true) {
      this.records_column = [
        {
          title: this.translate.instant('directDebits.mandateReference'),
          dataKey: 'mandate',
        },
        {
          title: this.translate.instant('directDebits.personalName'),
          dataKey: 'personalName',
        },
        { title: this.translate.instant('directDebits.bank'), dataKey: 'bank' },
        {
          title: this.translate.instant('directDebits.payerAccount'),
          dataKey: 'account',
        },
        {
          title: this.translate.instant('directDebits.amount'),
          dataKey: 'amount',
        },
        {
          title: this.translate.instant('directDebits.claimDate'),
          dataKey: 'claimDate',
        },
      ]
    } else if (this.currentItem['directDebit'] != true) {
      this.records_column = [
        {
          title: this.translate.instant('directDebits.claim'),
          dataKey: 'sequentialNumber',
        },
        {
          title: this.translate.instant('directDebits.mandateReference'),
          dataKey: 'payerCodeMandate',
        },
        {
          title: this.translate.instant('directDebits.personalName'),
          dataKey: 'payerHeading',
        },
        {
          title: this.translate.instant('directDebits.bank'),
          dataKey: 'payerBankCode',
        },
        {
          title: this.translate.instant('directDebits.payerAccount'),
          dataKey: 'payerAccount',
        },
        {
          title: this.translate.instant('directDebits.amount'),
          dataKey: 'amount',
        ...
        {
          title: this.translate.instant('directDebits.dueDate'),
          dataKey: 'dueDate',
        },
      ]
    }
  ...
  printPdf() {
    const columns = []
    const doc = this.generatePdf2().getBlob(
      (blob) => {
  ...
  getXlsx() {
    ...
    const records_row = this.prepareRows(this.records_column, this.records_row)
    ...
    const detail2_rows = this.getTableDetailSummary(tAlignment)
    const detail2_rows_data = []
    //console.log(detail2_rows);
    for (let i = 0; i < detail2_rows.length; i++) {
      const data = detail2_rows[i]
      detail2_rows_data.push({
        A: data[0].text,
        B: data[1].text,
        C: '',
        D: '',
    ...
    XLSX.utils.sheet_add_json(ws, detail2_rows_data, {
      skipHeader: true,
      origin: { r: 8, c: 0 },
    })
    ...
    XLSX.utils.sheet_add_json(ws, records_row, {
      skipHeader: false,
      origin: { r: level_rows.length + 18, c: 0 },
    })

    this.changeHeaderTitle(ws, this.level_column, 16)
    this.changeHeaderTitle(ws, this.records_column, level_rows.length + 19)
        ...
        { text: '\n' },
        {
          columns: [
            { width, text: '' },
            {
              ...
              table: {
                headerRows: 1,
                body: this.getTableDetailSummary(tAlignment),
              },
              layout: {
                defaultBorder: false,
                // fillColor(i, node) {
  ...
  private getTable(talign, columns, rows) {
    ...
    for (let j = 0; j < rows.length; j++) {
      ...
      for (let k = 0; k < columns.length; k++) {
        ...
        //     value_transformed = this.directDebitService.retrieveValue(rows[j][columns[k]["dataKey"]]);
        // }
        // else{
  ...
  private getTableDetail(talign) {
    ...
    if (this.currentItem['directDebit'] == true) {
      ...
      result.push(
        this.getLine(
          this.getDetailTitle(
            this.translate.instant('directDebits.originatorId'),
            talign,
          ),
          this.getDetailText(this.currentItem['originatorId'], talign),
          talign,
        ),
      )
      result.push(
        this.getLine(
          this.getDetailTitle(
            this.translate.instant('directDebits.batchName'),
            talign,
          ),
      ...
      const claimDate = new DateFormatPipe(
        this.injector,
        this._locale,
      ).transform(this.currentItem['claimDate'], 'dd/MM/yyyy')
      result.push(
        this.getLine(
          this.getDetailTitle(
            this.translate.instant('directDebits.claimDate'),
            talign,
          ),
          this.getDetailText(claimDate, talign),
          talign,
        ),
      )
      const initiationDate = new DateFormatPipe(
        this.injector,
        this._locale,
      ...
      result.push(
        this.getLine(
          this.getDetailTitle(
            this.translate.instant('directDebits.initiationDate'),
            talign,
          ),
          this.getDetailText(initiationDate, talign),
          talign,
        ),
      )
      result.push(
        this.getLine(
          this.getDetailTitle(
            this.translate.instant('directDebits.originatorName'),
            talign,
          ),
          this.getDetailText(this.currentItem['originatorName'], talign),
          talign,
        ),
      )

      const status_str = this.directDebitService.retrieveValue(
      ...
      const dueDate = new DateFormatPipe(this.injector, this._locale).transform(
      ...
      result.push(
        this.getLine(
          this.getDetailTitle(
            this.translate.instant('directDebits.dueDate'),
            talign,
          ),
          this.getDetailText(dueDate, talign),
          talign,
        ),
      )
      const initiationDate = new DateFormatPipe(
        this.injector,
        this._locale,
      ).transform(this.currentItem['initiationDate'], 'yyyy-MM-dd')
      result.push(
        this.getLine(
          this.getDetailTitle(
            this.translate.instant('directDebits.initiationDate'),
      ...
      result.push(
        this.getLine(
          this.getDetailTitle(
            this.translate.instant('directDebits.systemFileName'),
            talign,
          ),
          this.getDetailText(this.currentItem['systemFileName'], talign),
          talign,
        ),
      )
      result.push(
        this.getLine(
          this.getDetailTitle(
            this.translate.instant('directDebits.userFileName'),
            talign,
          ),
          this.getDetailText(this.currentItem['userFileName'], talign),
          talign,
        ),
      )
      const status_str = this.directDebitService.retrieveValue(
  ...
  private getTableDetailSummary(talign) {
    const result = []

    //const payrollUploadBatchDTO = this.currentItem['payrollUploadBatchDTO'];
    let line1 = this.getLine(
      this.getDetailTitle(
        this.translate.instant('directDebits.numberRajhiClaims'),
        talign,
      ),
      this.getDetailText(
        this.currentItem['directDebitFeesOutput'].numRajhiTransfers,
        talign,
      ),
      talign,
    )
    line1.push([], [], [], [])
    const line1_2 = this.getLine(
      this.getDetailTitle(
        this.translate.instant('directDebits.totalAmountRajhiClaims'),
        talign,
      ),
      this.getDetailText(
        this.currentItem['directDebitFeesOutput'].totalNumRajhi,
        talign,
      ),
      talign,
    )
    line1 = line1.concat(line1_2)
    //-----------------------------------------------------------
    let line2 = this.getLine(
      this.getDetailTitle(
        this.translate.instant('directDebits.numberNonRajhiClaims'),
        talign,
      ),
      this.getDetailText(
        this.currentItem['directDebitFeesOutput'].numNonRajhiTransfers,
        talign,
      ),
      talign,
    )
    line2.push([], [], [], [])
    const line2_2 = this.getLine(
      this.getDetailTitle(
        this.translate.instant('directDebits.totalNonAmountRajhiClaims'),
        talign,
      ),
      this.getDetailText(
        this.currentItem['directDebitFeesOutput'].totalNumNonRajhi,
        talign,
      ),
      talign,
    )
    line2 = line2.concat(line2_2)
    //-----------------------------------------------------------
    let line3 = this.getLine(
      this.getDetailTitle(
        this.translate.instant('directDebits.numberClaims'),
        talign,
      ),
      this.getDetailText(
        this.currentItem['directDebitFeesOutput'].numEmployees,
        talign,
      ),
      talign,
    )
    line3.push([], [], [], [])
    const line3_2 = this.getLine(
      this.getDetailTitle(
        this.translate.instant('directDebits.totalAmount'),
        talign,
      ),
      this.getDetailText(
        this.currentItem['directDebitFeesOutput'].totalAmount,
        talign,
      ),
      talign,
    )
    line3 = line3.concat(line3_2)
    //-----------------------------------------------------------
    let line4 = this.getLine(
      this.getDetailTitle(
        this.translate.instant('directDebits.feesAmountRajhiClaims'),
        talign,
      ),
      this.getDetailText(
        this.currentItem['directDebitFeesOutput'].alRajhiCustomersFeeSuccess,
        talign,
      ),
      talign,
    )
    line4.push([], [], [], [])
    const line4_2 = this.getLine(
      this.getDetailTitle(
        this.translate.instant('directDebits.totalFeesRajhiClaims'),
        talign,
      ),
      this.getDetailText(
        this.currentItem['directDebitFeesOutput'].totalFeesRajhi,
        talign,
      ),
      talign,
    )
    line4 = line4.concat(line4_2)
    //-----------------------------------------------------------
    let line5 = this.getLine(
      this.getDetailTitle(
        this.translate.instant('directDebits.feesAmountNonRajhiClaims'),
        talign,
      ),
      this.getDetailText(
        this.currentItem['directDebitFeesOutput'].nonAlRajhiCustomersFeeSuccess,
        talign,
      ),
      talign,
    )
    line5.push([], [], [], [])
    const line5_2 = this.getLine(
      this.getDetailTitle(
        this.translate.instant('directDebits.totalFeesNonRajhiClaims'),
        talign,
      ),
      this.getDetailText(
        this.currentItem['directDebitFeesOutput'].totalFeesNonRajhi,
        talign,
      ),
      talign,
    )
    line5 = line5.concat(line5_2)
    //-----------------------------------------------------------
    result.push(line1)
    result.push(line2)
    result.push(line3)
