...
import { CashManagementPoolingDetailService } from './cash-management-pooling-detail.service'
...
@UntilDestroy()
@Component({
  templateUrl: './cash-management-pooling-detail.component.html',
  styleUrls: ['./cash-management-pooling-detail.component.scss'],
  providers: [{ provide: 'Window', useValue: window }],
})
export class CashManagementPoolingDetailComponent
  ...
  implements OnInit, OnDestroy
{
  ...
  poolingChildAccountList: any[] = []

  public print = false

  constructor(
    public fb: FormBuilder,
    public service: CashManagementPoolingDetailService,
  ...
  ngOnInit() {
    ...
    if (this.structure == null) {
      ...
      this.staticService.getAllCombos(['currencyIso']).subscribe((data) => {
        this.service.setCurrencyCodesCombo(data[0].values)
        ...
        this.subscriptions.push(
          this.service.initStructure({}).subscribe((response) => {
            ...
            this.poolingChildAccountList = response.poolingChildAccountList

            this.subscriptions.push(
              this.service.getTreeItemsObserver().subscribe((treeItems) => {
                this.service.calculateNodesBalances(treeItems)
  ...
  onFinish() {
    this.router.navigate(['/cashManagement/pooling/list'])
  }
