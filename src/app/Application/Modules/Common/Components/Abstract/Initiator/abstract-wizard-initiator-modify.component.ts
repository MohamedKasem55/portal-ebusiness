...
@Directive()
export abstract class AbstractWizardInitiatorModifyComponent
  extends AbstractWizardInitiatorComponent
  implements OnInit
{
  @Input() markAllFormFieldAsTouched = true
  ...
  ngOnInit() {
    ...
    if (this.entityProperties) {
      ...
      if (this.markAllFormFieldAsTouched) {
        ...
        this.service.confirm(this.validationResponse).subscribe((result) => {
          if (result && result.errorCode) {
            if (+result.errorCode === 0) {
              this.markNextWizardStep()
              this.existError = false
              this.onSuccess.emit({ result, postData: this.validationResponse })
            } else {
              this.onError(result)
            }
          }
