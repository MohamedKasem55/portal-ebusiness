...
export class RequestReactivateService {
  ...
  public validate(batch): Observable<any> {
    ...
    const data = {
      businessCardPaymentBatchDSO: batch,
    }
    return this.http
      .post(this.servicesUrl + this.route + 'validate', data)
      .pipe(
        map((response: any) => {
          const body = response
  ...
  public save(batch, requestValidate, batchOld): Observable<any> {
    const data = {
      businessCardPaymentBatch: batch,
      requestValidate,
      oldBusinessCardPaymentBatch: batchOld,
    }
    return this.http.post(this.servicesUrl + this.route + 'confirm', data).pipe(
      map((response: any) => {
        const body = response
        if (response.errorCode !== '0') {
          const exception: Exception = new Exception(
  ...
  public delete(batch): Observable<any> {
    ...
    return this.http.post(this.servicesUrl + this.route + 'delete', data).pipe(
      map((response: any) => {
        const body = response
        if (response.errorCode !== '0') {
          const exception: Exception = new Exception(
