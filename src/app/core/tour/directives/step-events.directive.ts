...
// @dynamic
@Directive({
  selector: '[stepEventARB]',
})
export class StepEventsDirective implements OnInit {
  @Input('stepEventARB') eventType: string // possible values 'next', 'prev', 'close'
  isBrowser: boolean
  ...
  handleClose() {
    return (this.handler = () => {
      if (
        this.tourService.getLastStep().index + 1 ===
        this.tourService.getLastStep().total
      ) {
        this.done.emit({
          tourEvent: 'done',
